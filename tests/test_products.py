...
def test_mc_matches_analytic(product):
    ...
    assert r_mc.stderr is not None
...
def test_rainbow_upside_participation_affects_price():
    ...
    p05 = price_at(0.5)
    p20 = price_at(2.0)
    assert p05 is not None and p20 is not None
    assert p05 != p20
...
def test_rainbow_lower_call_strike_affects_price():
    ...
    def price_at(lcs):
        return RainbowNote(
            put_strike=0.9,
            lower_call_strike=lcs,
            upside_participation=1.0,
            guaranteed_flat_coupon=0.04,
            maturity_year_fraction=0.5,
        ).analytic_price(MARKET)

    p_low = price_at(0.95)
    p_high = price_at(1.05)
...
def test_rainbow_validation():
    with pytest.raises(ValueError):
        RainbowNote(
            put_strike=1.0,
            lower_call_strike=0.9,
            upside_participation=1.0,
            guaranteed_flat_coupon=0.0,
            maturity_year_fraction=0.5,
        )
...
def test_autocall_mc_converges():
    ...
    large = MonteCarloEngine(n_paths=200_000, batch_size=20_000, seed=81).price(
        ac, model
    )
