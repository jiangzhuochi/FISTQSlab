...
def main() -> None:
    ...
    print(
        f"Put-Call parity: C-P = {c - p:10.6f}  vs S - K·e^-rT = {S - K * np.exp(-r * T):10.6f}"
    )
    ...
    # 有限差分校验 delta/gamma
    h = 1e-4
    delta_fd = (bs_call(S + h, K, T, r, sigma) - bs_call(S - h, K, T, r, sigma)) / (
        2 * h
    )
    gamma_fd = (
        bs_call(S + h, K, T, r, sigma) - 2 * c + bs_call(S - h, K, T, r, sigma)
    ) / h**2
